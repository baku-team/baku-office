...
| ID | 指摘 | 方針 | 重さ |
|----|------|------|------|
| **3-1（本対応）** | ゼロ設定とSecret必須の衝突 | アクティベーションフローに Worker Secret(MASTER_KEY) 投入を組み込む or 規約・READMEにKV保管リスクを正式文書化（プロダクト判断＝段階）。**当面は上記の恒久警告で可視化** | 中〜大 |
...
- [ ] **005 §3 ゲート3点**：3-3 セッション失効（コード✓）／3-1 鍵保護（Secret必須化 or リスク文書化のいずれかを確定）／3-2 更新鍵ピン留め（別PRで実装）。
