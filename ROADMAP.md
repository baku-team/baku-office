...
- [ ] **会員本人のセルフサービス開示・削除請求フロー**（対象＝ログインユーザー `users`）。削除の挙動（即時 vs 管理者への請求）を確定のうえ実装。
...
- [x] **005 §3 ゲート3点（すべて対応済み）**：3-3 セッション失効（コード✓）／3-1 鍵保護（✓ ゼロ設定許容＋法務開示で文書化）／3-2 更新鍵ピン留め（✓ 実装＋CIガード）。
