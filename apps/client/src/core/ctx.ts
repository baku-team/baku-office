...
import { cfSqlStore, cfStorage, cfAi, cfAgent, cfGoogle } from "./cf-adapter.ts";
...
export function buildCtx(env: Env): Ctx {
  ...
  const ctx = { profile: detectProfile(env).id, env, db: cfSqlStore(env), storage: cfStorage(env), ai: cfAi(env), google: cfGoogle(env) } as Ctx;
