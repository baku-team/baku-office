...
export async function listSessions(ctx: Ctx, owner: string): Promise<SessionRow[]> {
  return await ctx.db.all<SessionRow>("SELECT id,title,model,updated_at FROM chat_sessions WHERE owner=? ORDER BY updated_at DESC LIMIT 50", [owner]);
}
export async function createSession(ctx: Ctx, owner: string, model?: string): Promise<string> {
  const id = randomId();
  const now = nowSec();
  await ctx.db.run("INSERT INTO chat_sessions (id,owner,title,model,created_at,updated_at) VALUES (?,?,?,?,?,?)",
...
export async function ownedSession(ctx: Ctx, owner: string, id: string): Promise<{ id: string; model: string | null } | null> {
  return (await ctx.db.first<{ id: string; model: string | null }>("SELECT id,model FROM chat_sessions WHERE id=? AND owner=?", [id, owner])) ?? null;
}
export async function getMessages(ctx: Ctx, sessionId: string): Promise<MsgRow[]> {
  return await ctx.db.all<MsgRow>("SELECT role,content,created_at FROM chat_messages WHERE session_id=? ORDER BY created_at LIMIT 200", [sessionId]);
}
export async function appendMessage(ctx: Ctx, sessionId: string, role: "user" | "assistant", content: string): Promise<void> {
  await ctx.db.run("INSERT INTO chat_messages (id,session_id,role,content,created_at) VALUES (?,?,?,?,?)",
...
export async function ensureTitle(ctx: Ctx, sessionId: string, firstText: string): Promise<void> {
  await ctx.db.run("UPDATE chat_sessions SET title=? WHERE id=? AND (title IS NULL OR title='')",
