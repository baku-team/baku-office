...
test("expectedBalance は opening_balance＋日付≤asOf の増減に一致（出納帳ロジック整合）", async () => {
  const { env } = setup();
  await createTx(env, { fiscal_period_id: "p1", date: "2026-05-01", wallet_id: "w-cash", kind: "income", category_id: "c-sales", amount: 1000, description: null, counter_wallet_id: null });
  await createTx(env, { fiscal_period_id: "p1", date: "2026-05-10", wallet_id: "w-cash", kind: "expense", category_id: "c-supply", amount: 300, description: null, counter_wallet_id: null });
  const { expectedBalance } = await import("../src/lib/register.ts");
  assert.equal(await expectedBalance(env, "p1", "w-cash", "2026-05-05"), 1000); // 5/10の出金は対象外
  assert.equal(await expectedBalance(env, "p1", "w-cash", "2026-05-31"), 700);
});
