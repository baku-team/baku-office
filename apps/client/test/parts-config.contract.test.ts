...
test("鍵保管の本番ブロック判定（§10.1）：本番でsecret未投入は missing-prod", () => {
  // 本番(ENVIRONMENT=production)＋secret未投入＝暗号処理ブロック中。
  assert.equal(detectProfile({ ENVIRONMENT: "production" } as never).keyStore, "missing-prod");
  // 本番でも secret 投入済みなら secret。
  assert.equal(detectProfile({ ENVIRONMENT: "production", MASTER_KEY: "k" } as never).keyStore, "secret");
  // dev/test(ENVIRONMENT未設定)は従来どおり KV 自動生成。
  assert.equal(detectProfile({} as never).keyStore, "kv-autogen");
});
