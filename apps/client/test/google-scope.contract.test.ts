...
test("googleAuthUrl：選んだグループの scope だけを要求（gmail送信を含めない）", () => {
  const url = googleAuthUrl(ENV, "https://x.example", "st", ["gmail_read"])!;
  ...
  // 閲覧＋送信
  const url2 = googleAuthUrl(ENV, "https://x.example", "st", ["gmail_read", "gmail_send"])!;
  assert.deepEqual(scopesOf(url2), ["https://www.googleapis.com/auth/gmail.modify", "https://www.googleapis.com/auth/gmail.send"]);
});

test("googleAuthUrl：offline + consent + state を設定", () => {
  const u = new URL(googleAuthUrl(ENV, "https://x.example", "STATE", ["calendar"])!);
