...
test("fileBelongsTo：register_invoice 経路の所有者検査（他人/不在は false）", async () => {
  const { env } = setup();
  assert.equal(await fileBelongsTo(env, "p1", "line:u1"), true);   // 本人
  assert.equal(await fileBelongsTo(env, "p1", "line:u2"), false);  // 他人
  assert.equal(await fileBelongsTo(env, "orgA", "admin1"), true);  // 本人(org)
  assert.equal(await fileBelongsTo(env, "missing", "admin1"), false); // 不在
});
