...
export const POST: APIRoute = async ({ request }) => {
  const b = (await request.json().catch(() => ({}))) as { token?: string; query?: string; queryEmbedding?: number[] | null; tags?: string[]; limit?: number; certifiedOnly?: boolean };
  ...
  const results = await searchEntries(env, {
    ...
    certifiedOnly: b.certifiedOnly === true,
  });
  // 自団体は検索結果から除外。
