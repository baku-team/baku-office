...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  if (b._action === "approve") { await approve(env, b.license_id); return json({ ok: true }); }
  if (b._action === "reject") { await reject(env, b.license_id, String(b.reason ?? "")); return json({ ok: true }); }
