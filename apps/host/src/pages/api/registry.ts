...
import { listApps, registerApp, setAppStatus, usageByApp } from "../../lib/registry.ts";
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  const b = (await request.json().catch(() => ({}))) as { _action?: string; id?: string; name?: string; version?: string; repoUrl?: string; publisher?: string; category?: string; permissions?: string[]; description?: string; status?: string };
