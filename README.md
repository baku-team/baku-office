...
**パーツ＝再利用可能アプリ**（`id`/`name`/`version`）：①特定団体専用でなく**複数団体で共有**（各団体は「有効パーツ」で ON/OFF）、②アプリ更新（`version` 上げ）はコア配布で**導入先すべてに波及**（自動マイグレーション）、③既存アプリをコピーし `id` を変えて改変＝**派生で新アプリ**（`derivedFrom` に派生元）。詳細は [§14-7](baku-office_portable-core_architecture.md)。
...
**申込時はプランを選ばない＝全員 Free で開始**。AI(Plus)・エージェント(Pro) へのアップグレードは導入後に管理画面「プラン・課金」(`/billing`) から行う。プラン別ゲート（標準パーツの提供範囲）：
