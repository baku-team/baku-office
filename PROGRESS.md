...
baku-office の実装トラッカー（生きた管理ファイル）。設計詳細は [03](docs/archive/03_multitenant-saas-architecture.md)、スタックは [05](docs/archive/05_implementation-stack.md)。
...
- **2026-06：baku-office として再設計（正本＝[integrated_design_package_v1.0.md](docs/spec/integrated_design_package_v1.0.md)）**。位置づけ＝**「自社専用AIの相棒」をクライアントが丸ごと所有する「ポータブルコア＋パーツ」基盤**（会計・庶務はパーツの一例）。実装は `apps/host`・`apps/client`（Astro on Workers・自己ホスト型）。旧LINEエージェント（`worker/`）は apps/client へ統合済みのため削除（git履歴に残存）。
...
- **実装済み（2026-06-10 追加分・本番反映済み #38）**：
  - **レビュー指摘の改善**：①無人エージェントジョブ（cron自動巡回・owner=org/role=member）で対外/破壊系ツールを**道具レベルで遮断**（`AgentTool.unattended`＝`send_message`/`update_event`/`delete_event` を非提示＋実行拒否、子エージェント・A2A対外道具も除外、`processAgentJobs` は `unattended`）＝メール本文によるプロンプトインジェクションの余地を排除（プロンプト依存を廃止）。②自動取込(owner=org)の**期日通知**が届かない問題を解消＝アプリ内通知（**migration 0021** `notifications`）＋**任意Webhook**（Discord互換 content/text）で配信、ヘッダのベル🔔＋未読バッジ＋設定にWebhook欄。③**CI 配布前 typecheck ゲート**（release.yml/publish-client.yml）。④**Web チャットの画像/PDF添付UI**（📎→base64→/api/chat image）。
  - **Google 連携＝Workspace 組織アカウント前提を明記**（アクティベート画面）：対象＝法人・非営利団体・事業体（個人事業主含む）。Workspace 組織アカウントは審査不要、個人アカウント（@gmail.com 等）は Google 審査が必要となり得て**可否は Google 側の判断**。
  - 検証：全パッケージ typecheck 0 errors・client test 36/36（notifications 契約テスト追加）。本番 client（`baku-office-app`・env.production・Version `8dd9274a`）反映、**migration 0021 自動適用を本番D1で確認**（SCHEMA_VERSION=21）。
...
- **実装済み（2026-06-09 第三者レビュー全改善・Phase1〜6）**：別途の第三者レビュー（セキュリティ／機能正当性／UI-UX）で検出した約30件を重要度順に全改修。新規 D1 マイグレーションなし。
  - **🔴 Critical**：無認証活性化の取り残し（`/api/activate`・`/api/token` を `ENV=development` 限定にゲート＋callback 検証）／nonprofit 却下が課金団体を free 転落させるバグ（`entitlementForPlan` 新設）／公開 throwaway リポの deploy_code 露出（deploy_url 受領時に即削除）。
  - **🟠 Security**：SSRF 強化（FQDN 必須・credentials 拒否・A2A fetch を redirect:manual）／submit の id 乗っ取り防止（予約id拒否＋所有者不一致409＋監査）／A2A リプレイ防止（nonce 使い捨て）・app-action 権限経路の明示・レート計上を ok のみ（自己ロック解消）／セッション鍵を HKDF サブ鍵で暗号化鍵と分離。
  - **🟠 機能**：キルスイッチで `app_drafts` も無効化／Stripe 昇格時の nonprofit 保護／自口座振替の 400 拒否／autopilot・agent の entitlement 鮮度ゲート（解約後 Pro 残存窓の縮小・Profile C 維持）。
  - **🟡 機能**：gh_merge_pr の files/check-runs 全ページ取得（コア領域/失敗チェックの見逃し防止）／CSV 符号付金額列／課金 past_due でのダウングレード／0011 の非冪等を注記。
  - **UI/UX**：host nonprofit/apps を `window.bo.api` に統一／apply デプロイ待ちのタイムアウト導線／sticky ヘッダ解除／会計入力のクライアント検証／エラートースト assertive・login ラベル／顧客削除のモーダル化／`/audit` の検索・フィルタ・ページング／申込画面のブランド色統一。
  - 検証：host 13 / client 14（増分0）・client test 35/35・3 app build green。詳細は [baku-office_review_確認事項と改善点.md](docs/archive/baku-office_review_確認事項と改善点.md) 第2次レビュー対応表。
- **実装済み（2026-06-09 追加分・ホスト統制＋自己修復・本番反映済み）**：
  - **アプリ統制の強化**：ストア/未登録アプリの**公開停止（blocked）・削除（deleted）**を全クライアントへ反映（`buildCheck` が `registry_apps` の blocked/deleted ＋墓標 `app_revocations` を統合して `revokedApps` 配布）。削除は方針「**墓標＋利用0で完全削除**」（利用申告0なら registry 行も物理削除、残れば status=deleted で履歴保持）。**未登録で稼働中**のアプリも `/apps` から登録／公開停止／復帰／削除可。`/api/registry` に delete/revoke/unrevoke 追加。
  - **標準同梱アプリの登録／除外**：ホストが同梱パーツの有効/無効を `builtin_policy` で制御→`disabledBuiltins` を統合チェックで配布→クライアントの導入集合・エージェント道具から除外（必須=chat は除外不可）。`BUILTIN_APP_IDS` を9アプリへ拡張（site/import/branding の未登録誤検知も解消）。
  - **プラン/エンタイトルメントのUI整理**：データは両方維持（plan=契約/請求・entitlement=実効権限。nonprofit審査/Stripe入金が並行上書きするため統合不可）。`/clients` で **entitlement を主（緑バッジ）・plan を従** に再構成（「どちらを見るか」を解消）。
  - **自己修復ログ（client→host→GitHub）**：クライアントのエラーは `logDiag(error)` が送信アウトボックスに積み、`cron/drain` の `flushReports` が host `/api/report` へ集約送信（license token認証・fingerprint集約）。利用者の不具合/要望は `/diagnostics` のフォーム→`/api/report`。ホストは `client_reports` に集積し `/reports` で統制、「**GitHubへ集積**」で `baku-team/baku-office-logs` に Issue 化（→ 人間が Claude に巡回・修復させる分担）。対応（resolved/wontfix＋メモ/PR）は `reportUpdates` でクライアントへ返信表示。
  - **定期巡回スケジューラ（新Worker）**：`apps/scheduler/`＝**Cloudflare Cron Triggers `*/5 * * * *`**。Service Binding 経由（同一 workers.dev 直fetchは 1042 で遮断）で host `/api/cron/sweep`（報告→GitHub集積）と client `/api/cron/drain`（reports flush 他）を起動。外部スケジューラ非依存・CF内で完結。`CRON_TARGETS` 秘匿設定。
  - 前提：host D1 に `0012_governance.sql`（手動remote）／client は `0018_reports.sql`（自動）。host secret `INTERNAL_KEY` 追加、host var `GITHUB_LOGS_REPO`、`GITHUB_TOKEN` に Issues:write。検証：client test 35/35・host 4/4・新規コードtypeクリーン。本番（host/client/scheduler）反映・sweep/drain 各200 疎通。
- **実装済み（2026-06-09 追加分・Google Workspace連携 #34・本番反映済み）**：
  - **Google Workspace連携（標準同梱パーツ4種・Pro以上）**：`src/parts/` に **gmail/calendar/meet/invoices** を追加（表示名 Gmail／カレンダー／Meet議事録／請求書、画面 `/gmail` `/calendar` `/meet` `/invoices`）。1度の OAuth 同意で Gmail/Calendar/Meet を一括接続（`google.ts`・refresh_token は `apikey:google_refresh` に MASTER_KEY 暗号化保管）。Gmail＝受信検索・本文/添付取得・送信、カレンダー＝予定CRUD＋Meetリンク発行、Meet議事録＝録画トランスクリプト取得→Claude要約→組織ナレッジ保存＋タスク化、請求書＝ファイルからAI抽出（`media-ai.ts extractInvoiceData`）・未払一覧・消込・期日リマインド。各パーツはエージェント道具としても提供。
  - **マイグレーション**：client D1 に `0019_google.sql`（`meet_records`）／`0020_invoices.sql`（`invoices`）を追加。**SCHEMA_VERSION=20**。`migrate.ts` の `ensureSchema` が初回リクエストで未適用分のみ自動適用（`schema_migrations`＋KV `schema_version` ゲート。wrangler `d1_migrations` は不使用）。
  - 検証：`npm run typecheck` 全パッケージ 0 errors、client test 35/35・host 4/4。既存の型負債（shared `crypto.ts`/`tsconfig`、App/Host インラインscript）も解消。
  - 反映：client は env.production（baku-llc・`baku-office-app`）へ `wrangler deploy --env production`、host は `baku-office-portal`。初回アクセスで 0019/0020 自動適用を確認（`meet_records`・`invoices` 作成）。
- **環境メモ（2026-06-09 時点）**：開発・運用は **baku-llc の Cloudflare アカウント（`*.baku-027.workers.dev`）のみ**。client wrangler の top-level（コメント上「amber-links 動作確認」）は現在不使用で、実ターゲットは **env.production（baku-llc）**。稼働 Worker は host=`baku-office-portal`／client=`baku-office-app`／apply=`baku-office-apply`／scheduler=`baku-office-scheduler`。本文中の「本番反映済み」は当社（baku-llc）env.production への deploy を指し、**外部顧客向けの本番提供はまだ開始していない（開発段階）**。
- **次の一手**：本番クレデンシャル投入（Google/LINE/Discord/Stripe/Gemini/Claude）、`PUBLISH_TOKEN`(配布CI)、**セキュリティ3ゲート（KMS署名・FIDO2・admin JIT＝引き続き保留）**、2社目の実テナントE2E、法務確定。残：gemini/claude の ChatModel ライブ検証、他Part画面の `ctx` 完全移行、**配布テンプレへの cron 同梱**（顧客アカウントでの client 自走drain）。
...
最終更新：2026-06-13（**ROADMAP未完項目のコード対応＋社名統一＋配布v0.2.2**。内容：(1) 脆弱性報告窓口 `SECURITY.md`＋`apps/client/public/.well-known/security.txt`（`.txt`＝未認証配信）。(2) 単価未登録API（Web検索/画像/音声/動画/任意API）を使用量画面で「**推定不可**」表示＝$0誤認の防止（`config.ts hasPricing()`・USD上限入力の無効化・推定費用合計から除外を注記）。(3) **IDORランタイムテストを chat-sessions へ横展開**（owner越え読取/削除の拒否・履歴連動削除・`getMessages` の呼出側ガード契約／`test/chat-sessions-idor.contract.test.ts`・client 123→127 pass）。(4) バックアップ画面に **D1 Time Travel(30日)＋会計CSV書き出し案内＋団体責任**を追記。(5) OPERATIONS **A-8 にコードのロールバック手順**（旧コードを新バージョンで再リリース＝ロールフォワード）。(6) 全資料の社名を「**合同会社貘**」へ統一（20箇所）。検証：typecheck 0／client 127 pass。**本番 client（`baku-office-app`・env.production・Version `3630c16c`）反映**、配布 **VERSION 0.2.1→0.2.2**・**baku-office-app に v0.2.2 署名リリース公開**＋host `/api/release/latest` が 0.2.2 返却を確認。CI 3本 success。PR #99）／2026-06-13（**Astro 6＋アダプタv13 移行＋クラウドAI強化を本番反映**。内容：(1) **Astro 5→6＋`@astrojs/cloudflare` 13系へ更新**＝保留中だった P2-1 を完了。env アクセスを `import { env } from "cloudflare:workers"` へ全面移行（旧 `Astro.locals.runtime.env` は廃止・約136ファイル）、実行コンテキストは `Astro.locals.cfContext`（旧 `runtime.ctx`）。配布バンドルは v13構成＝`server/`（`entry.mjs`＋事前バンドル）＋`client/`（静的アセット）に分離・`no_bundle`・`base_dir ./server`。本番デプロイは `deploy:prod`（`CLOUDFLARE_ENV=production astro build && wrangler deploy`／v13が `dist/server/wrangler.json` リダイレクト設定を生成）。`npm audit`(--omit=dev) clean。(2) **Workers AI モデル廃止対応**＝`@cf/meta/llama-3.1-8b-instruct`（2026-05-30廃止）→ `-instruct-fast` を既定に。(3) **クラウドAIの上位モデル選択**＝`WORKERS_AI_MODELS`（標準=8b-instruct-fast／高性能=70b-fp8-fast）を管理者がAI画面で選択、KV設定に保存（解決順 KV設定>env>既定）。(4) **通信量（KV書込）の可視化＋リセット時刻**＝CFは実行時に書込回数を出さないため `op_usage`(D1) に自前日次カウント、全KV書込を `kvPut` に集約（**migration 0028／SCHEMA_VERSION=28**）、使用量ページ・ホーム管理者向け詳細に「本日 N/1000・%バー・UTC0時リセット・80%警告」を表示。PR #85〜#91。typecheck0／client 113 pass。本番反映済み・migration 0028 自動適用確認）／2026-06-12（**UI刷新＋相棒キャラ＋Workers AI 一連を本番反映**。統合設計書 §15／事業説明資料／ご案内資料を更新（v1.5）。内容：貘ブランドのデザインシステム＋ダークテーマ＋ナビ3形態＋新トップバー、相棒マスコット（貘キャラ・差し替え/AI生成[flux]・稼働可視化[/api/activity]・エンジン別アニメ・Pro未満休眠）、Cloudflare Workers AI（`env.AI`・クラウド無料AI・API未登録時の既定・Gemini/Claude障害時の自動フォールバック＋事情説明・prompt形式/ツールなし）、ニューロン使用量の可視化＋上限、全データバックアップ（D1+KV+ファイル／ローカル・Drive／復元・7日アラート・/api/backup）、体感速度（clientPrerender/prefetch・ホーム並列化+疎通非ブロック+KPIキャッシュ・上部進捗バー）、DBインデックス追加 **migration 0027／SCHEMA_VERSION=27**、ログインレート制限・ベータ規約・確認ダイアログ網羅、AIの内部構造非開示の鉄則＋自己認識の最新化。PR #57〜#82。typecheck0／client tests pass）／2026-06-10（#46＝第三者レビュー003 是正＋code-review追加是正。**本番反映済み**（client/host を env.production へ手動デプロイ・migration 0025 files.ctx 自動適用・SCHEMA_VERSION=25 確認済）。内容：ファイルIDOR（ctx分離＋ロール・getFileForSession等／register_invoice の所有者検査／LINE個人ファイルのctx=personal）、dev-confirm/checkout 本番fail-closed、公開HTML allowlistサニタイザ＋公開CSP（&Tab;/&NewLine;バイパス・属性&エスケープ・srcset候補別検査・O(n²)是正含む）、使用量記録失敗の診断化、API認可lint導入＋未認証API是正（keys.ts/tx.ts→requireOrgAdmin集約）、本番起動preflight（boot-check）。PR #46。typecheck0／client80／host8 pass）／2026-06-10（#38＝レビュー指摘改善：無人ジョブの対外/破壊系ツール道具レベル遮断・org期日通知のアプリ内通知＋任意Webhook（migration 0021・SCHEMA_VERSION=21）・CI配布前typecheckゲート・Webチャット画像/PDF添付UI・Google連携=Workspace組織アカウント前提の明記。本番client反映済み）／2026-06-09（#16〜#34 を反映＋第三者レビュー全改善 Phase1〜6。Google Workspace連携（gmail/calendar/meet/invoices・migration 0019/0020・SCHEMA_VERSION=20）＋型負債解消を追加。マルチエージェント／A2A／マーケット／NonProfit／オートパイロット＋運用機能 S6〜S12 に加え、レビュー指摘の約30件（活性化認証の取り残し・nonprofit却下バグ・公開リポ deploy_code・SSRF/submit所有権/A2A nonce・権限・レート・セッション鍵分離・キルスイッチ拡張・nonprofit×Stripe・自口座振替・autopilot鮮度・gh_merge_prページング・課金past_due・UI/UX統一/a11y）を改修。KMS署名/FIDO2/admin JIT は引き続き保留）
