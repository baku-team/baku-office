...
- **2026-06：baku-office として再設計（正本＝[integrated_design_package_v1.0.md](integrated_design_package_v1.0.md)）**。会計・庶務SaaS＋LINEエージェントへ。実装は `apps/host`・`apps/client`（Astro on Workers・自己ホスト型）。旧LINEエージェントは `worker/` に温存。
- **実装済み**：申込/ライセンス/自動アクティベート、会計コア、マルチユーザー（招待・承認・ロール・PII暗号・共有承認）、ファイル/予定/議事録、Stripe（デモ可）、認証OAuth（dev併用）、LINEエージェント＋各AI機能（要約/音声/web検索/資料生成/任意API/スキル）、診断・Workers Paid案内、UI統一・レスポンシブ、自動マイグレーション、配布CI。実CFで一気通貫検証済み。
- **次の一手**：本番クレデンシャル投入（Google/LINE/Discord/Stripe/Gemini/Claude）、`PUBLISH_TOKEN`(配布CI)、**セキュリティ3ゲート（KMS署名・FIDO2・admin JIT）**、2社目の実テナントE2E、法務確定。
- **方針**：共通更新はupstream/CIで全顧客へ、カスタムはデータ駆動で個別（[ARCHITECTURE.md](ARCHITECTURE.md)）。バックアップは顧客自己責任。

### 📋 今後の追加予定（バックログ）

- [ ] **Googleドライブ連携**（顧客アカウント単位・OAuthスコープ追加）
  - [ ] ドライブ内ファイルのメタ情報（id/名前/MIME/サイズ/更新日時/親フォルダ等）をクライアントD1に同期（差分/定期）
  - [ ] 同期メタを使ったファイル検索・参照（リスト化）UIとエージェント参照
  - [ ] 自動バックアップ：KV・R2 の内容を Google ドライブへ定期バックアップ（**任意設定**・スケジュール/対象選択可）。バックアップ顧客自己責任方針と整合（顧客のドライブへ退避できる選択肢を提供）

- [ ] **プラン名称変更：X→Free／Y→Plus／Z→Pro**（UI・ドキュメント・コード内エンタイトルメント表記を一括更新。内部キーの変更要否は実装時に判断）

- [ ] **AIチャット画面（Plus以上）**
  - [ ] 集計・DB/ファイル検索・ファイルを元にした文書作成を Web UI 上で実行
  - [ ] 生成物は **DL可能な形で出力（PDF／TXT／md／HTML／CSV）**

- [ ] **AI/エージェントによるスキル検索・反映（Plus以上）**：AI/エージェントでスキルを検索し、Agent Skills に自動設定

- [ ] **「API使用量」画面（Plus以上）**
  - [ ] AI機能含む各APIの使用量を数値＋グラフで表示
  - [ ] 無料枠超過の恐れがある場合にアラート（**リセット時刻も表示**）
  - [ ] 従量課金は上限値を設定可能（上限到達時＝**無料版に切替 または 該当API利用を一時停止**）

- [ ] **高度なオプションは Plus 以上でのみ表示**

- [ ] **ホスト：団体申込みを別Worker（申込専用）に分離**（団体管理画面から切り出し）

- [ ] **ホスト：クライアントリストの強化**
  - [ ] 団体情報の詳細参照（代表者・連絡先・登録年月日 等）
  - [ ] 各種情報・ステータスを管理画面から**手動変更**可能に
...
最終更新：2026-06-04（2Worker分離＋多テナント台帳＋他アカウント対応をライブ稼働。バックアップはクライアント自己責任へ方針変更。KMS署名/FIDO2/admin JIT は課題として保留）
