...
- [ ] **HP/LP 作成・公開機構（Pro以上）** ※「仕組みだけ」用意し、希望者のみ都度実装
  - [ ] ホスト側で個別にHP/LPを作成→クライアント側でセルフ公開（他システム同様の個別作成・共有・セルフ導入機構）
  - [ ] HP管理画面・会員管理画面をクライアント管理画面に組込み、HPと会員管理を連動
  - [ ] Stripe連動（任意）：カード支払で **新規会員追加／ステータス変更／退会(サブスク解約)時の削除** を自動連携。現金管理の団体は手動またはエージェントで会員管理

- [ ] **Notion / Googleドライブからの資料インポート（Plus以上想定）**
  - [ ] **既定はメタデータのみインポート**（実ファイルは取り込まない）
  - [ ] インポート前に **DB・ストレージの容量シミュレーション＋費用試算**。容量不足時は対策（Workers Paid/R2有効化等）を提示
  - [ ] **R2有効時のみ** オプションでバイナリ（実ファイル）を全てR2へインポート可
